...
export function calculateSAW(alternatives, criteriaList, userWeights) {
  ...
  // Final Score = Sum of (Normalized Value * User Weight for that Criteria)
  
  // Calculate total user weight to normalize weights if needed, 
  // but standard SAW just multiplies. Let's use direct multiplication.
  ...
  const finalResults = normalizedMatrix.map(row => {
    ...
    criteriaList.forEach(c => {
      const userWeight = parseFloat(userWeights[c.id]) || c.weight || 0; // fallback to default criteria weight
      const weightedValue = row.values[c.id] * userWeight;
